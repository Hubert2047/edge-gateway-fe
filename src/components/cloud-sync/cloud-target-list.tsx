...
} from '@/lib/api/cloud-target.queries'
...
type FormErrors = Partial<Record<'name' | 'apiBaseUrl' | 'cloudServerId' | 'cloudServerSecret' | 'uploadIntervalSeconds', string>>
...
function validateForm(form: CloudTargetFormValues): FormErrors {
    ...
    if (!form.uploadIntervalSeconds || form.uploadIntervalSeconds <= 0) errors.uploadIntervalSeconds = '請輸入有效的秒數'
    return errors
}
...
export function CloudTargetList({ initialTargets }: { initialTargets: CloudTarget[] }) {
    ...
    function runTestConnection(id: string) {
        ...
        testMutation.mutate(id, {
            ...
            onError: () =>
                setTestResults((prev) => ({ ...prev, [id]: { success: false, message: '連線失敗' } })),
        })
    }
        ...
        <div className="flex h-full flex-col gap-4 overflow-hidden">
                                    ...
                                    >
                                        ...
                                        <td className="p-4 space-y-3">
                                            <div className="space-y-1.5">
                                                <Label htmlFor={`name-${target.id}`} className="text-xs text-muted-foreground">顯示名稱</Label>
                                                <Input
                                                    id={`name-${target.id}`}
                                                    value={form.name}
                                                    disabled={rowBusy}
                                            ...
                                            <div className="space-y-1.5">
                                                <Label htmlFor={`serverid-${target.id}`} className="text-xs text-muted-foreground">雲端服務器</Label>
                                                <Input
                                                    id={`serverid-${target.id}`}
                                                    className={`font-mono ${errors.cloudServerId ? 'border-destructive' : ''}`}
                                                    value={form.cloudServerId}
                                                    disabled={rowBusy}
                                                    onChange={(e) => updateRowForm(target.id, { cloudServerId: e.target.value })}
                                                />
                                                {errors.cloudServerId && <p className="text-xs text-destructive">{errors.cloudServerId}</p>}
                                            </div>
                                            <div className="space-y-1.5">
                                                <Label htmlFor={`secret-${target.id}`} className="text-xs text-muted-foreground">雲端服務器密鑰</Label>
                                                <Input
                                                    id={`secret-${target.id}`}
                                                    className={errors.cloudServerSecret ? 'border-destructive' : ''}
                                                    value={form.cloudServerSecret}
                                                    disabled={rowBusy}
                                                    onChange={(e) => updateRowForm(target.id, { cloudServerSecret: e.target.value })}
                                                />
                                                {errors.cloudServerSecret && <p className="text-xs text-destructive">{errors.cloudServerSecret}</p>}
                                            </div>
                                        </td>
                                        <td className="p-4 space-y-3">
                                            <div className="space-y-1.5">
                                                <Label htmlFor={`url-${target.id}`} className="text-xs text-muted-foreground">API BASE URL</Label>
                                                <Input
                                                    id={`url-${target.id}`}
                                                    value={form.apiBaseUrl}
                                                    disabled={rowBusy}
                                                    onChange={(e) => updateRowForm(target.id, { apiBaseUrl: e.target.value })}
                                            ...
                                            <div className="flex gap-3 flex-col">
                                                <div className="w-28 shrink-0 space-y-1.5">
                                                    <Label htmlFor={`interval-${target.id}`} className="text-xs text-muted-foreground">上傳頻率</Label>
                                                    <div className="flex items-center gap-1.5">
                                                        <Input
                                                            ...
                                                            disabled={rowBusy}
                                                            onChange={(e) => updateRowForm(target.id, { uploadIntervalSeconds: Number(e.target.value) })}
                                                            className={errors.uploadIntervalSeconds ? 'border-destructive' : ''}
                                                        />
                                                        <span className="text-xs text-muted-foreground">秒</span>
                                                    </div>
                                                    {errors.uploadIntervalSeconds && <p className="text-xs text-destructive">{errors.uploadIntervalSeconds}</p>}
                                                </div>
                                                <div className="min-w-0 flex-1 pt-4 space-y-1 text-xs text-muted-foreground">
                                                    <div className='flex gap-4'>
                                                        ...
                                                        <p className='font-bold'> {target.lastUploadAt ? formatRelativeTime(target.lastUploadAt) : '尚未上傳'}</p>
                                                    </div>
                                                    <div className='flex gap-4'>
                                                        <p>待續傳</p>
                                        ...
                                        <td className="p-4">
                                            <div className="flex flex-col items-end gap-1.5">
                                                <Button size="sm" className="w-20" disabled={saving || testing} onClick={() => requestSave(target)}>
                                                ...
                                                >
                                                    {testing ? <Loader2 className="h-4 w-4 animate-spin" /> : '測試連線'}
                                                </Button>
                                                <Button
                                                    size="sm"
                                                ...
                                                {testResult && (
                                                    <p className={`text-xs ${testResult.success ? 'text-emerald-600' : 'text-rose-600'}`}>
                                                        {testResult.success ? '連線成功' : testResult.message || '連線失敗'}
                                                    </p>
                                                )}
                                            </div>
                                        </td>
            ...
            <Card className="relative shrink-0 px-4 sm:px-6 pt-4 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-0">
                    ...
                    <div className="space-y-1.5">
                        <Label htmlFor="new-url" className="text-xs text-muted-foreground">API BASE URL</Label>
                        <Input
                            id="new-url"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-0">
                    <div className="space-y-1.5">
                        <Label htmlFor="new-serverid" className="text-xs text-muted-foreground">雲端服務器 ID</Label>
                        <Input
                            id="new-serverid"
                        ...
                        />
                        {newErrors.cloudServerId && <p className="text-xs text-destructive">{newErrors.cloudServerId}</p>}
                    </div>
                    <div className="space-y-1.5">
                        <Label htmlFor="new-secret" className="text-xs text-muted-foreground">雲端服務器密鑰</Label>
                        <Input
                            id="new-secret"
                            type="password"
                        ...
                        />
                        {newErrors.cloudServerSecret && <p className="text-xs text-destructive">{newErrors.cloudServerSecret}</p>}
                    </div>
                    <div className="space-y-1.5">
                        <Label htmlFor="new-interval" className="text-xs text-muted-foreground">上傳頻率（秒）</Label>
                        <Input
                            id="new-interval"
                            type="number"
                            placeholder="60"
                        ...
                        />
                        {newErrors.uploadIntervalSeconds && <p className="text-xs text-destructive">{newErrors.uploadIntervalSeconds}</p>}
                    </div>
                </div>
                <Button disabled={createMutation.isPending} onClick={createTarget} size="lg" className="w-full sm:w-max">
                    {createMutation.isPending ? (
                        <>
                            <Loader2 className="h-4 w-4 animate-spin" />
                            新增中...
