...
export function ProcessControlAnalysis({ hubs, meters }: Props) {
    ...
    function handleHubChange(value: string) {
        setHubUid(value)
        const firstMeter = meters.find((meter) => meter.hubUID === value)
    ...
    return (
        <div className='flex min-h-full flex-col gap-7 pb-8'>
            <div className='flex items-center justify-between border-b border-[#D8DDD9] pb-5'>
                ...
                <span className='rounded-full bg-[#EAF5EF] px-4 py-2 text-sm font-medium text-[#357A59]'>{t('processControl.normal')}</span>
            </div>
            ...
            <section className='border border-[#D8DDD9] bg-white p-4'>
                <div className='grid gap-3 xl:grid-cols-7'>
                    ...
                    <Field label={t('processControl.lowerLimit')}>
                        <input value={lowerLimit} onChange={(event) => setLowerLimit(event.target.value)} inputMode='decimal' className='control-input text-right' />
                    </Field>
                    <Field label={t('processControl.upperLimit')}>
                        <input value={upperLimit} onChange={(event) => setUpperLimit(event.target.value)} inputMode='decimal' className='control-input text-right' />
                    </Field>
                </div>
                <button type='button' className='mt-3 flex h-8 w-32 items-center justify-center gap-2 bg-[#153F31] px-2.5 text-sm font-medium text-white hover:bg-[#1B503D]'>
                ...
                <div className='h-[26rem] w-full'>
                    <ResponsiveContainer width='100%' height='100%'>
                        <LineChart data={emptyChartData} margin={{ top: 20, right: 18, left: 18, bottom: 8 }}>
                            ...
                            <YAxis domain={[Number(lowerLimit) || 0, Number(upperLimit) || 1]} tick={{ fill: '#7B8580', fontSize: 12 }} tickLine={false} axisLine={false} />
                            <Tooltip />
                            ...
                            <Line type='monotone' dataKey='value' stroke='#287A5F' strokeWidth={3} dot={false} connectNulls={false} />
                        </LineChart>
                    </ResponsiveContainer>
                </div>
                <div className='mt-3 flex justify-end gap-5 text-xs text-[#7B8580]'>
                    <span>UCL {upperLimit}</span><span>CL —</span><span>LCL {lowerLimit}</span>
                </div>
...
function Summary({ label, value }: { label: string; value: string }) {
    return <div className='border-b border-r border-[#D8DDD9] px-5 py-6 last:border-r-0 md:border-b-0'><p className='text-sm text-[#7B8580]'>{label}</p><p className='mt-7 font-mono text-3xl font-semibold'>{value}</p></div>
}
