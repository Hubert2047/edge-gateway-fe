...
type FormErrors = Partial<Record<'meterName' | 'macId' | 'voltage' | 'powerFactor', string>>

function validateForm(form: MeterFormValues, opts: { isNew: boolean }): FormErrors {
  const errors: FormErrors = {}
  if (!form.meterName.trim()) errors.meterName = '請輸入名稱'
...
export function MeterList({ hubUid, initialMeters }: { hubUid: string; initialMeters: Meter[] }) {
  const { data: meters = initialMeters } = useMeters(hubUid, initialMeters)
  ...
  function toggleEnabled(meter: Meter, checked: boolean) {
    updateMeterMutation.mutate(
      { macId: meter.macId, enabled: checked },
  ...
  function requestSave(meter: Meter) {
    ...
    setPendingAction({ type: 'save', macId: meter.macId, displayName: form.meterName || meter.macId })
  }

  function requestDelete(meter: Meter) {
    const { form } = getRowForm(meter)
    setPendingAction({ type: 'delete', macId: meter.macId, displayName: form.meterName || meter.macId })
  }
  ...
  async function confirmPendingAction() {
    ...
    if (action.type === 'saveAll') {
      const updates = action.macIds.map((macId) => {
  ...
  function createMeter() {
    ...
    createMeterMutation.mutate(newForm, {
      onSuccess: () => {
    ...
    <div className="flex h-full flex-col gap-4 overflow-hidden">
      <Card className="shrink-0 px-4 sm:px-6 pt-4 space-y-4">
        <div className="flex items-center justify-between mb-0">
          ...
          <span className="text-sm text-muted-foreground">{meters.length} 個</span>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-0">
          <div className="space-y-1.5">
            <Label htmlFor="new-name" className="text-xs text-muted-foreground">名稱</Label>
            <Input
              id="new-name"
              placeholder="例如：主進線"
              value={newForm.meterName}
              onChange={(e) => updateNewForm({ meterName: e.target.value })}
              className={newErrors.meterName ? 'border-destructive' : ''}
            />
            {newErrors.meterName && <p className="text-xs text-destructive">{newErrors.meterName}</p>}
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="new-mac" className="text-xs text-muted-foreground">MAC ID</Label>
            <Input
              id="new-mac"
          ...
          <div className="space-y-1.5">
            ...
            <Select
              value={newForm.meterType}
              onValueChange={(v) => {
                if (!v) return
                updateNewForm({ meterType: v })
              }}
            >
              <SelectTrigger>
                <SelectValue>{phaseLabel[newForm.meterType]}</SelectValue>
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-1.5">
            <Label htmlFor="new-voltage" className="text-xs text-muted-foreground">設定電壓 (V)</Label>
            <Input
              id="new-voltage"
              type="number"
          ...
          <div className="space-y-1.5">
            <Label htmlFor="new-pf" className="text-xs text-muted-foreground">功率因數</Label>
            <Input
              id="new-pf"
              type="number"
              step="0.01"
        ...
        <Button disabled={createMeterMutation.isPending} onClick={createMeter} size="lg" className="w-full sm:w-max">
      ...
      <Card className="flex flex-1 min-h-0 flex-col overflow-hidden pt-0 border">
        <div className="flex items-center justify-between border-b px-4 py-3">
          ...
          <Button size="sm" disabled={dirtyCount === 0 || bulkMutation.isPending} onClick={requestSaveAll}>
            {bulkMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : `儲存全部 (${dirtyCount})`}
          </Button>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {meters.map((meter) => {
                ...
                const saving = updateMeterMutation.isPending && updateMeterMutation.variables?.macId === meter.macId
                const deleting = deletingMacId === meter.macId
                const rowBusy = saving || deleting || bulkMutation.isPending
                return (
                  <tr
                    key={meter.macId}
                  ...
                  >
                    <td className="p-2">
                      <Input
                        ...
                        disabled={rowBusy}
                        onChange={(e) => updateRowForm(meter.macId, { meterName: e.target.value })}
                        className={errors.meterName ? 'border-destructive' : ''}
                      />
                      {errors.meterName && <p className="text-xs text-destructive mt-1">{errors.meterName}</p>}
                    </td>
                    <td className="p-2">
                    ...
                    <td className="p-2">
                      <Select
                        ...
                        onValueChange={(v) => {
                          if (!v) return
                          updateRowForm(meter.macId, { meterType: v })
                        }}
                      >
                        <SelectTrigger>
                          <SelectValue>{phaseLabel[form.meterType]}</SelectValue>
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="three_phase">三相</SelectItem>
                          <SelectItem value="single_phase">單相</SelectItem>
                    ...
                    <td className="p-2">
                      <Input
                        ...
                        value={form.voltage}
                        disabled={rowBusy}
                        onChange={(e) => updateRowForm(meter.macId, { voltage: Number(e.target.value) })}
                    ...
                    <td className="p-2">
                      <Input
                        ...
                        disabled={rowBusy}
                        onChange={(e) => updateRowForm(meter.macId, { powerFactor: Number(e.target.value) })}
                        className={errors.powerFactor ? 'border-destructive' : ''}
                      />
                      {errors.powerFactor && <p className="text-xs text-destructive mt-1">{errors.powerFactor}</p>}
                    </td>
                    ...
                    <td className="p-2">
                      <div className="flex items-end gap-1.5">
                        <Button size="sm" className="w-16" disabled={rowBusy} onClick={() => requestSave(meter)}>
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              className={pendingAction?.type === 'delete' ? 'bg-rose-600 text-white hover:bg-rose-700' : ''}
            >
              確認
            </AlertDialogAction>
          </AlertDialogFooter>
